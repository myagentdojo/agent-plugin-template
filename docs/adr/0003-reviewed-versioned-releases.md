...
Public and private Git-repository canaries qualify publishing-system changes, not every recipient release. They bind both GitHub API identity and real Git transport identity, publish one unique `refs/heads/candidate/<sha>` per candidate, wait for hosted proof, and compare native client caches. Candidate refs are never moved, force-pushed, deleted, or reused for another commit.
