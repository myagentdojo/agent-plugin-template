...
- **Audience is first-party and self-hosted.** The plugins are authored by us
  and run on our own machines. We trust the code, so a runtime that *cannot*
  touch the OS protects against a threat we do not have. The sandbox was the
...
- **Keep two tiers** (ADR 0004): a real security boundary for untrusted,
  third-party plugins, but this audience is first-party, so the boundary
  guards a threat we do not face, at the cost of a permanent second runtime.
...
- Every plugin can touch the OS. That is acceptable for first-party,
  self-hosted use and explicitly deferred to architecture-layer isolation
