...
- KTD4. Implement only the narrow top-level `stop_hook_active` boolean extraction needed by the contract using POSIX `awk` lexical state. Do not use raw grep, parse string/nested decoys, or create a general JSON parser.
