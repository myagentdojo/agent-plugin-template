...
4. Run the shared handler directly once for `Stop` with the active client and `stop_hook_active` set to false. Capture only pass, mismatch, or operational failure. Do not return raw handler output.
